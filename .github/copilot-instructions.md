...
- **PyLint on specific folder**: `pylint homeassistant/components/vmc_helty_flow`
- **MyPy type checking (whole project)**: `mypy homeassistant/`
- **MyPy on specific integration**: `mypy homeassistant/components/vmc_helty_flow`
...
```
homeassistant/components/vmc_helty_flow/
...
def mock_device_api() -> Generator[MagicMock]:
    ...
    with patch("homeassistant.components.vmc_helty_flow.MyDeviceAPI", autospec=True) as api_mock:
...
async def init_integration(
    ...
    with patch("homeassistant.components.vmc_helty_flow.PLATFORMS", platforms):
        await hass.config_entries.async_setup(mock_config_entry.entry_id)
        await hass.async_block_till_done()
...
caplog.set_level(logging.DEBUG, logger="vmc_helty_flow")
...
python -m script.hassfest --integration-path homeassistant/components/vmc_helty_flow

# Validate quality scale
# Check quality_scale.yaml against current rules

# Run integration tests with coverage
pytest ./tests/components/vmc_helty_flow \
  --cov=homeassistant.components.vmc_helty_flow \
  --cov-report term-missing
```

# VMC Helty Flow Integration Guidelines
Guidelines for developing and maintaining the VMC Helty Flow integration in Home Assistant are described in requirements.md file.

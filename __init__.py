...
class VmcHeltyCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry):
        ...
        self._normal_update_interval = timedelta(
            seconds=config_entry.options.get("scan_interval", DEFAULT_SCAN_INTERVAL.total_seconds())
        )
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            try:
                ...
                _LOGGER.warning("Timeout durante l'acquisizione dello stato da %s: %s", self.ip, err)
                self._handle_error()
            ...
            if not status_response or not status_response.startswith("VMGO"):
                self._handle_error()
                raise UpdateFailed(f"Dispositivo {self.ip} non risponde correttamente: {status_response}")
            ...
            try:
                ...
                _LOGGER.warning("Impossibile leggere le info di rete da %s: %s", self.ip, err)

            # Aggiornamento riuscito, reset del contatore errori
            if self._consecutive_errors > 0:
                _LOGGER.info("Ripristinata connessione con %s dopo %d errori consecutivi",
                             self.ip, self._consecutive_errors)
            ...
            if self.update_interval != self._normal_update_interval:
                ...
                _LOGGER.info("Ripristinato intervallo normale di aggiornamento per %s", self.ip)
            ...
            self._handle_error()
            _LOGGER.exception("Errore imprevisto durante l'aggiornamento dei dati per %s: %s", self.ip, err)
    ...
    def _handle_error(self):
        ...
        if self._consecutive_errors == 1:
            ...
            _LOGGER.error(
                ...
                self._max_consecutive_errors, self.ip
            )
        elif self._consecutive_errors > self._max_consecutive_errors:
            _LOGGER.debug("Errore consecutivo #%d per %s", self._consecutive_errors, self.ip)
        else:
            _LOGGER.info("Errore consecutivo #%d per %s", self._consecutive_errors, self.ip)

        # Se abbiamo raggiunto il limite, passa alla modalità di ripristino
        if (self._consecutive_errors >= self._max_consecutive_errors and
            ...
            _LOGGER.info(
                ...
                self.ip, self._error_recovery_interval.total_seconds()
            )
            ...
            try:
                ...
                if len(parts) > 1 and parts[1].strip():
                    ...
                    if new_name != self.name:
                        _LOGGER.info("Nome dispositivo cambiato da '%s' a '%s'", self.name, new_name)
                        self.name = new_name
                ...
                _LOGGER.warning("Errore durante l'aggiornamento del nome dispositivo: %s", err)

...
def test_parse_vmsl_response_pure():
    ...
    ssid, password = helpers.parse_vmsl_response(response)
    assert ssid == "ValentinoNet"
    assert password == "Password"

def test_get_device_name_pure():
    assert helpers.get_device_name("192.168.1.8") == "VMC Helty 8"
    assert helpers.get_device_name("10.0.0.123") == "VMC Helty 123"
